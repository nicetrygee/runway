...
3. **Module refactor (behaviour-preserving).** Split the single `app.py` so parallel slices touch different files. Move code, don't rewrite it; keep the CSRF, session, rate-limit, logging, and error-handler wiring exactly as-is.
   ...
   |---|---|---|
   | `db.py` | every `db.execute(...)` query, wrapped in named functions (e.g. `tasks_for_user`, `insert_task`, `update_task`, `set_status`, `delete_task`, `completed_since`) | all slices |
   | `classify.py` | `ExtractedTask`, `WeeklySummary`, `extract_task_from_text`, `generate_weekly_summary`, the `ai_client` setup | Slice A / D |
   | `events.py` | *new* — `log_event(user_id, event_type, item_id=None, person_id=None, payload=None)` and read helpers | C / D |
...
4. **`last_touched_at` on every write.** `insert_task`, `update_task`, and `set_status` in `db.py` set `last_touched_at = CURRENT_TIMESTAMP` and append a matching `events` row via `log_event` (`created` / `touched` / `status_changed`, and `completed` when status becomes `done`).
